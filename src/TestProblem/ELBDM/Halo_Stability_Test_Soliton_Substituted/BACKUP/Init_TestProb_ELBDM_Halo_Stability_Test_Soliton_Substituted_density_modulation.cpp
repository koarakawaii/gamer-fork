/* Substitute the Halo center region with the analytical soliton function */
...
// This function computes desnity profile, with standare deviation 
...
extern void Aux_ComputeCorrelation( Profile_t *Correlation[], const Profile_with_Sigma_t *prof_init[], const double Center[],
...
// intern functions
static void Record_CenterOfMass( bool record_flag, int loop_terminated);
static real Soliton_DensProfile( real r );
//static void WaveFunc_ResetByUser_API( const int lv, const int FluSg, const double TimeNew, const double dt );
//static bool Reset_Central_Region( real fluid[], const double x, const double y, const double z, const double Time,
...
static double   CriteriaFactor;               // wave function inside radius<CriteriaFactor*CoreRadius will be substituted by wave function computed from analytical soltion density profile
static double   PhaseBetweenRealImag;         // wave function phase difference between real and imaginary part
static double   Redshift;                     // redshift z
...
// ********************************************************************************************************************************
   ...
   ReadPara->Add( "PhaseBetweenRealImag",     &PhaseBetweenRealImag,      0.0,            -2.0*M_PI,           2.0*M_PI      );
   ReadPara->Add( "Redshift",                 &Redshift,                  0.0,             0.0     ,       NoMax_double      );
...
// (4) make a note
   if ( MPI_Rank == 0 )
   {
      ...
      Aux_Message( stdout, "  criteria factor for defining central region = %13.6e\n", CriteriaFactor            );
      Aux_Message( stdout, "  phase between real and imaginary part       = %13.6e\n", PhaseBetweenRealImag      );
      Aux_Message( stdout, "  redshift z                                  = %13.6e\n", Redshift                  );
...
// Return      :  None
//-------------------------------------------------------------------------------------------------------
static void AddNewField_ELBDM_Halo_Stability_Test(void)
{
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Init_User_ELBDM_Halo_Stability_Test_Soliton_Substituted
// Description :  Store the initial density and substitute the center region with soliton
//
...
// Return      :  None
//-------------------------------------------------------------------------------------------------------
static void Init_User_ELBDM_Halo_Stability_Test_Soliton_Substituted(void)
{
   if ( OPT__RESTART_RESET == 1 )
   {
//    find the soliton center
      bool record_flag    = false;
      int loop_terminated = 1 ;
      ...
      real   scale_factor    = 1./(1.+Redshift);
      m_23                   = ELBDM_MASS*UNIT_M/(Const_eV/SQR(Const_c))/1.0e-23;
      soliton_density_factor = 1.9e9/(double)scale_factor*pow(m_23,-2.)*pow((double)CoreRadius*(double)UNIT_L/3.08567758e21,-4.)*1.98847e33/pow(3.08567758e21,3.)/(double)UNIT_D;
      //      //      soliton_density_factor  = 1.9e9/(double)scale_factor*pow(m_23,-2.)*pow((double)CoreRadius*(double)UNIT_L/3.08567758e21,-4.)*1.98847e33/pow(3.08567758e21,3.)/(double)UNIT_D;
      for (int lv=0; lv<NLEVEL; lv++)
      {
         const double dh = amr->dh[lv];
#  pragma omp parallel for private( dr, x, y, z, x0, y0, z0, r, SolitonDens ) schedule( runtime )
            ...
            for (int k=0; k<PS1; k++)
            {
               ...
               for (int j=0; j<PS1; j++)
               {
                  ...
                  for (int i=0; i<PS1; i++)
                  {
                     ...
                     if ( r<=r_criteria )  
                     {
//                        printf(" r = %.6e ; soliton density = %.6e \n", r, SolitonDens);
//                        real fluid_dens = amr->patch[ amr->FluSg[lv] ][lv][PID]->fluid[DENS][k][j][i];
//                        printf("Effective !! r = %.6e ; r_criteria = %.6e ; original density = %.6e ;", r, r_criteria, fluid_dens);
                        amr->patch[ amr->FluSg[lv] ][lv][PID]->fluid[DENS][k][j][i] = SolitonDens;
                        amr->patch[ amr->FluSg[lv] ][lv][PID]->fluid[REAL][k][j][i] = SQRT(SolitonDens)*COS(PhaseBetweenRealImag);
                        amr->patch[ amr->FluSg[lv] ][lv][PID]->fluid[IMAG][k][j][i] = SQRT(SolitonDens)*SIN(PhaseBetweenRealImag);
//                        fluid_dens = amr->patch[ amr->FluSg[lv] ][lv][PID]->fluid[DENS][k][j][i];
//                        printf(" soliton density = %.6e \n", fluid_dens);
                     }
                     else
                     {
                        amr->patch[ amr->FluSg[lv] ][lv][PID]->fluid[REAL][k][j][i] = SQRT(SolitonDens)*COS(PhaseBetweenRealImag)*EXP((real)1.-r/r_criteria) + (amr->patch[ amr->FluSg[lv] ][lv][PID]->fluid[REAL][k][j][i])*((real)1.-EXP((real)1.-r/r_criteria));
                        amr->patch[ amr->FluSg[lv] ][lv][PID]->fluid[IMAG][k][j][i] = SQRT(SolitonDens)*SIN(PhaseBetweenRealImag)*EXP((real)1.-r/r_criteria) + (amr->patch[ amr->FluSg[lv] ][lv][PID]->fluid[IMAG][k][j][i])*((real)1.-EXP((real)1.-r/r_criteria));
                        amr->patch[ amr->FluSg[lv] ][lv][PID]->fluid[DENS][k][j][i] = SQR(amr->patch[ amr->FluSg[lv] ][lv][PID]->fluid[REAL][k][j][i]) + SQR(amr->patch[ amr->FluSg[lv] ][lv][PID]->fluid[IMAG][k][j][i]);
                     }
		  } // end of for loop i
               } // end of for loop j
            } // end of for loop k
...
#  endif
   ...
   } // end of if ( OPT__RESTART_RESET == 1 )
...
//    b. for starting a new simulation, we must copy the initial density to both Sg
   ...
   if (ComputeCorrelation)
   {
       ...
       // compute the center position for passive field
...
static void GetCenterOfMass( bool record_flag, const double CM_Old[], double CM_New[], const double CM_MaxR )
{
   ...
   long DensMode;
   // if record_flag is True, calculate center of mass based on total denisty field; if record_flag is False, calculate center of mass based on passive field
...
// set an initial guess by the peak density position
       ...
       if ( (!record_flag) && (repeat==loop_terminated) )
       {
// loop_terminated = 0: Cached the center coordinate by CoM coordinate of the whole halo for passive field, when simuliation BEGINS!!
// loop_terminated = 1: Cached the center coordinate by CoM coordinate of the soliton halo for centeral region to be substituted, when simuliation BEGINS!!
// loop_terminated = 2: Normal. Do not cach anything.
           for (int i=0; i<3; i++)
               Center[i] = CM_New[i];
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Soliton_DensProfile
// Description :  Calculate the soliton density value at a given radius, according to the analytical formula
// Note        :  1. mass core ration should already included in the core radius, should not be explicitly used here
//                2. core raduis should shared the same unit system as UNIT_L
//                3. chage core radius from UNIT_L to kpc
// Paramter    :  CoreRadius: soliton core radius
//                ELBDM_MASS: fuzzy dark matter particle mass in unit of eV/c^2
//
//-------------------------------------------------------------------------------------------------------
real Soliton_DensProfile(real r)
{
    real x = r/CoreRadius; // core radius should shared the same unit system as UNIT_L
    return soliton_density_factor/(real)(pow(1.+pow(factor*x,2.),8.));  // density in UNIT_D
}
...
//                lv       : Target refinement level
//                AuxArray : Auxiliary array
//                CriteriaFactor: define inside how many times of core radius the wave function will replaced by that derived from analtyical soliton density profile
//                CoreRadius    : soltion core radius
//
// Return      :  true  : This cell has been reset
//                false : This cell has not been reset
////-------------------------------------------------------------------------------------------------------
//bool Reset_Centeral_Region( real fluid[], const double x, const double y, const double z, const double Time,
//                            const double dt, const int lv, double AuxArray[] )
//{
//   real SolitonDens;
//   real r_criteria     = CriteriaFactor*CoreRadius;
...
//      fluid[REAL] = SQRT(SolitonDens)*COS(PhaseBetweenRealImag);
//      fluid[IMAG] = SQRT(SolitonDens)*SIN(PhaseBetweenRealImag);
////      printf(" soliton density = %.6e ; real part = %.6e ; imag part = %.6e\n", fluid[DENS], fluid[REAL], fluid[IMAG]);
//   }
//   else
//   {
//      fluid[DENS] = SolitonDens;
//      fluid[REAL] = SQRT(SolitonDens)*COS(PhaseBetweenRealImag)*EXP((real)1.-r/r_criteria) + fluid[REAL]*((real)1.-EXP((real)1.-r/r_criteria));
//      fluid[IMAG] = SQRT(SolitonDens)*SIN(PhaseBetweenRealImag)*EXP((real)1.-r/r_criteria) + fluid[IMAG]*((real)1.-EXP((real)1.-r/r_criteria));
//   }
//   return true;
//}
...
//-------------------------------------------------------------------------------------------------------
//void WaveFunc_ResetByUser_API( const int lv, const int FluSg, const double TimeNew, const double dt )
//{
////   if ( TimeNew==0. )
//   if ( TimeNew==dt )
//   {
...
//      {
//         printf("Center position for reset central region is (%.6e,%.6e,%.6e) \n", Center[0], Center[1], Center[2]);
////          printf("soliton_density_factor = %.8e ; m_23 = %.8e ; UNIT_L = %.8e ; UNIT_D = %.8e \n", soliton_density_factor, m_23, UNIT_L, UNIT_D);
//      }
//      //
//
//// check
...
// set the problem-specific runtime parameters
   ...
   Init_User_Ptr            = Init_User_ELBDM_Halo_Stability_Test_Soliton_Substituted;
//   Flu_ResetByUser_API_Ptr  = WaveFunc_ResetByUser_API;
//   Flu_ResetByUser_Func_Ptr = Reset_Centeral_Region;
#  endif // #if ( MODEL == ELBDM  &&  defined GRAVITY )
