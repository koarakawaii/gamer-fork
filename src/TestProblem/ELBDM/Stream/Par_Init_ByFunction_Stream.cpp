...
extern int    Stream_Plummer_RSeed;
extern double Stream_Plummer_M;
extern double Stream_Plummer_R0;
extern double Stream_Plummer_MaxR;
extern double Stream_Plummer_Center[3];
extern double Stream_Plummer_BulkVel[3];
extern int    Stream_Plummer_MassProfNBin;
extern double Stream_Plummer_Rho0;
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Par_Init_ByFunction_Stream
...
// only the master rank will construct the initial condition
   if ( MPI_Rank == 0 )
   {
//    const double TotM_Inf = 4.0/3.0*M_PI*CUBE(Stream_Plummer_R0)*Stream_Plummer_Rho0;
      const double TotM_Inf = Stream_Plummer_M;
...
//    initialize the random number generator
      ...
      RNG->SetSeed( 0, Stream_Plummer_RSeed );


//    determine the total enclosed mass within the maximum radius
      TotM = MassProf_Plummer( Stream_Plummer_MaxR );
      ParM = TotM / NPar_AllRank;


//    construct the mass profile table
      Table_MassProf_r = new double [Stream_Plummer_MassProfNBin];
      Table_MassProf_M = new double [Stream_Plummer_MassProfNBin];

      dr = Stream_Plummer_MaxR / (Stream_Plummer_MassProfNBin-1);

      for (int b=0; b<Stream_Plummer_MassProfNBin; b++)
      {
...
//       --> sample from the cumulative mass profile with linear interpolation
         ...
         RanR = Mis_InterpolateFromTable( Stream_Plummer_MassProfNBin, Table_MassProf_M, Table_MassProf_r, RanM );
...
//       randomly set the position vector with a given radius
         ...
         for (int d=0; d<3; d++)    Pos_AllRank[d][p] = RanVec[d] + Stream_Plummer_Center[d];
...
//       determine the maximum velocity (i.e., the escaping velocity)
         Vmax = Vmax_Fac*pow( SQR(Stream_Plummer_R0) + SQR(RanR), -0.25 );
...
//       randomly set the velocity vector with the given amplitude (RanV*Vmax)
         ...
         for (int d=0; d<3; d++)    Vel_AllRank[d][p] = RanVec[d] + Stream_Plummer_BulkVel[d];
...
double MassProf_Plummer( const double r )
{
   ...
   const double x = r / Stream_Plummer_R0;

   return 4.0/3.0*M_PI*Stream_Plummer_Rho0*CUBE(r)*pow( 1.0+x*x, -1.5 );
