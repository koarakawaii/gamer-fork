...
// convert dh from double to real for various solvers
   const real dh_real[3] = { amr->dh[lv][0], amr->dh[lv][1], amr->dh[lv][2] };
...
// we adopt the minimum dh at level JEANS_MIN_PRES_LEVEL to calculate the pressure floor of Jeans length
   const real dh_min             = fmin(  fmin( amr->dh[JEANS_MIN_PRES_LEVEL][0], amr->dh[JEANS_MIN_PRES_LEVEL][1] ),
                                          amr->dh[JEANS_MIN_PRES_LEVEL][2]  );
   const real JeansMinPres_Coeff = ( JEANS_MIN_PRES ) ?
                                   NEWTON_G*SQR(JEANS_MIN_PRES_NCELL*dh_min)/(GAMMA*M_PI) : NULL_REAL;
#  else
...
#        ifdef GPU
         CUAPI_Asyn_FluidSolver( h_Flu_Array_F_In[ArrayID], h_Flu_Array_F_Out[ArrayID], h_DE_Array_F_Out[ArrayID],
                                 ...
                                 NPG, lv, dt, dh_real, GAMMA, OPT__FIXUP_FLUX, Flu_XYZ, OPT__LR_LIMITER, MINMOD_COEFF,
...
#        ifdef GPU
         CUAPI_Asyn_PoissonGravitySolver( h_Rho_Array_P[ArrayID], h_Pot_Array_P_In[ArrayID],
                                          ...
                                          NULL, NULL, NULL,
                                          NPG, lv, dt, dh_real, SOR_MIN_ITER, SOR_MAX_ITER,
...
#        ifdef GPU
         CUAPI_Asyn_PoissonGravitySolver( NULL, NULL,
                                          ...
                                          NPG, lv, dt, dh_real, NULL_INT, NULL_INT,
...
#        ifdef GPU
         CUAPI_Asyn_dtSolver( TSolver, h_dt_Array_T[ArrayID], h_Flu_Array_T[ArrayID], NULL, h_Corner_Array_T[ArrayID],
                              NPG, lv,          (Step==0)?DT__FLUID_INIT:DT__FLUID, GAMMA, MIN_PRES,
...
#        ifdef GPU
         CUAPI_Asyn_dtSolver( TSolver, h_dt_Array_T[ArrayID], NULL, h_Pot_Array_T[ArrayID], h_Corner_Array_T[ArrayID],
                              NPG, lv,          DT__GRAVITY, NULL_REAL, NULL_REAL, OPT__GRA_P5_GRADIENT, OPT__GRAVITY_TYPE, NULL_BOOL,
                              TimeNew, GPU_NSTREAM );
#        else
         CPU_dtSolver       ( TSolver, h_dt_Array_T[ArrayID], NULL, h_Pot_Array_T[ArrayID], h_Corner_Array_T[ArrayID],
                              NPG, lv, dh_real, DT__GRAVITY, NULL_REAL, NULL_REAL, OPT__GRA_P5_GRADIENT, OPT__GRAVITY_TYPE, NULL_BOOL,
                              TimeNew );
#        endif
      break;
#     endif
